...
class App extends Component {
    ...
    async loadBlockchainData(){
        ...
        const web3 = new Web3(new Web3.providers.HttpProvider("http://ec2-3-22-119-209.us-east-2.compute.amazonaws.com:8545"))
        var account;
        ...
        count = await this.state.contract.methods.totalVotesFor('Daniel').call(console.log);
	this.setState( { Daniel : parseInt(count) } )
    }
    constructor(props){
        super(props)
...
render(){
        ...
        return (
		 ...
		 <button onClick={() => this.voting("Daniel")}>Daniel vote</button>
		</div>

                <h5>Johnny count: {this.state.Johnny}</h5>
                <h5>Amber count: {this.state.Amber}</h5>
		<h5>Daniel count: {this.state.Daniel}</h5>
		</div>
        );
}
}
